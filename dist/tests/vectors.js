...
// Positive vectors
const uint64Zero = { kind: index_js_1.TypeKind.Basic, fixedSize: 8 };
const bytes32Type = { kind: index_js_1.TypeKind.Basic, fixedSize: 32 };
const listUint64 = { kind: index_js_1.TypeKind.List, elementType: uint64Zero };
const listBytes32 = { kind: index_js_1.TypeKind.List, elementType: bytes32Type };
// Test 1: uint64(0)
{
    const data = new Uint8Array(8);
    const res = (0, index_js_1.sszStreamRootFromSlice)(uint64Zero, data);
...
// Test 3: empty list<uint64>
{
    ...
    const res = (0, index_js_1.sszStreamRootFromSlice)(listUint64, data);
...
{
    const vectorType = { kind: index_js_1.TypeKind.Vector, elementType: uint64Zero };
    const data = new Uint8Array(16);
    data[0] = 1;
    data[8] = 2;
    const res = (0, index_js_1.sszStreamRootFromSlice)(vectorType, data);
...
// Test 12: container with non-increasing offsets
{
    ...
    data.set(offset2, 12);
    const res = (0, index_js_1.sszStreamRootFromSlice)(containerType, data);
    assert('error' in res && res.error === index_js_1.SszError.BadOffset, 'non-increasing offsets should fail');
}
// Test 13: trailing extra bytes
{
    const data = new Uint8Array(9);
    const res = (0, index_js_1.sszStreamRootFromSlice)(uint64Zero, data);
    assert('error' in res && res.error === index_js_1.SszError.NonCanonical, 'trailing bytes should fail');
}
// Test 14: offset points outside buffer
{
    ...
    assert('error' in res && res.error === index_js_1.SszError.BadOffset, 'offset beyond buffer should fail (misalignment)');
}
// Test 15: empty bitlist
{
    ...
    const data = new Uint8Array(0);
    const res = (0, index_js_1.sszStreamRootFromSlice)(bitlistType, data);
    assert('error' in res && res.error === index_js_1.SszError.NonCanonical, 'empty bitlist should fail');
}
...
{
    const badType = { kind: index_js_1.TypeKind.Basic };
    const data = new Uint8Array(8);
    const res = (0, index_js_1.sszStreamRootFromSlice)(badType, data);
    assert('error' in res && res.error === index_js_1.SszError.UnsupportedType, 'unsupported type should fail');
}
...
// Test 19: list<bytes32> trailing bytes
{
    ...
    data[32] = 0xff;
    const res = (0, index_js_1.sszStreamRootFromSlice)(listBytes32, data);
    assert('error' in res && res.error === index_js_1.SszError.NonCanonical, 'list trailing bytes should fail');
}
// Test 20: streaming reader test
{
    ...
    const res = (0, index_js_1.sszStreamRootFromReader)(uint64Zero, reader);
    assert('root' in res, 'streaming reader should succeed');
    const sliceRes = (0, index_js_1.sszStreamRootFromSlice)(uint64Zero, data);
