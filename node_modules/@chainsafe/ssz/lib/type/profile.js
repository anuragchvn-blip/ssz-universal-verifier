import { BranchNode, concatGindices, getNode, getNodesAtDepth, merkleizeBlocksBytes, subtreeFillToContents, toGindex, zeroHash, zeroNode, } from "@chainsafe/persistent-merkle-tree";
...
import { computeSerdesData, getProfileTreeViewClass, } from "../view/profile.js";
import { getProfileTreeViewDUClass, } from "../viewDU/profile.js";
 ...
 */
export class ProfileType extends CompositeType {
    fields;
    opts;
    typeName;
    depth;
    ...
    getFieldRanges(data, start, end) {
        ...
        const optionalFields = new BitArray(data.uint8Array.subarray(start, start + optionalFieldsByteLen), this.optionalFieldsCount);
        const { variableOffsetsPosition, fixedEnd, fieldRangesFixedLen, isFixedLen } = computeSerdesData(optionalFields, this.fieldsEntries);
        ...
        const offsets = readVariableOffsets(data.dataView, start, end, optionalFieldsByteLen, fixedEnd, variableOffsetsPosition);
 ...
 * Offsets are relative to the start of serialized active fields, after the Bitvector[N]
 */
function readVariableOffsets(data, start, end, optionalFieldsEnd, fixedEnd, variableOffsetsPosition) {
 ...
 */
export function renderContainerTypeName(fields, prefix = "Profile") {
    const fieldNames = Object.keys(fields);
