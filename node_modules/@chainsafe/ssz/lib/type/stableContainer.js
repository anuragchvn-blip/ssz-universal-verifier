import { BranchNode, LeafNode, concatGindices, countToDepth, getNode, getNodeH, getNodesAtDepth, merkleizeBlocksBytes, setNode, setNodeWithFn, subtreeFillToContents, toGindex, zeroHash, zeroNode, } from "@chainsafe/persistent-merkle-tree";
...
import { computeSerdesData, getContainerTreeViewClass, } from "../view/stableContainer.js";
import { getContainerTreeViewDUClass, } from "../viewDU/stableContainer.js";
import { CompositeType, isCompositeType } from "./composite.js";
import { isOptionalType, toNonOptionalType } from "./optional.js";
/**
 ...
 */
export class StableContainerType extends CompositeType {
    maxFields;
    opts;
    typeName;
    depth;
    ...
    tree_createProofGindexes(node, jsonPaths) {
        ...
        for (const jsonPath of jsonPaths) {
            ...
            if (!isCompositeType(type)) {
                ...
                const leafGindexes = type.tree_getLeafGindices(gindex, type.fixedSize === null ? getNode(node, gindex) : undefined);
    ...
    getFieldRanges(data, start, end) {
        ...
        const { variableOffsetsPosition, fixedEnd, fieldRangesFixedLen, isFixedLen } = computeSerdesData(activeFields, this.fieldsEntries);
 ...
 */
function readVariableOffsets(data, start, end, activeFieldsEnd, fixedEnd, variableOffsetsPosition) {
 ...
 */
export function precomputeJsonKey(fieldName, casingMap, jsonCase) {
    if (casingMap) {
 ...
 */
export function renderContainerTypeName(fields, prefix = "StableContainer") {
 ...
 */
export function getActiveFields(rootNode, bitLen) {
...
export function setActiveFields(rootNode, activeFields) {
    // fast path for depth 1, the bitvector fits in one chunk
...
export function getActiveField(rootNode, bitLen, fieldIndex) {
...
export function setActiveField(rootNode, bitLen, fieldIndex, value) {
...
export function mixInActiveFields(root, activeFields, output, offset) {
    // fast path for depth 1, the bitvector fits in one chunk
