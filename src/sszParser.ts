...
export function parseToRanges(
  td: TypeDesc,
  bytes: Uint8Array
): { ranges: Range[]; error: SszError; msg: string } {
  if (td.kind === TypeKind.Basic) {
    if (td.fixedSize === undefined)
      return { ranges: [], error: SszError.UnsupportedType, msg: 'Basic type missing fixedSize' };
    if (td.fixedSize === 0) {
      // Variable-length bytes
      return { ranges: [{ start: 0, end: bytes.length }], error: SszError.None, msg: '' };
    }
    if (bytes.length !== td.fixedSize)
      return { ranges: [], error: SszError.NonCanonical, msg: 'Basic type length mismatch' };
    return { ranges: [{ start: 0, end: bytes.length }], error: SszError.None, msg: '' };
  }

  if (td.kind === TypeKind.Bitlist) {
    if (bytes.length === 0)
      return { ranges: [], error: SszError.NonCanonical, msg: 'Bitlist empty' };
    const lastByte = bytes[bytes.length - 1];
    if (lastByte === 0)
      return { ranges: [], error: SszError.BitlistPadding, msg: 'Bitlist sentinel missing' };
    ...
    if (paddingBits >= 31)
      return { ranges: [], error: SszError.BitlistPadding, msg: 'Bitlist padding overflow' };
    const mask = (1 << paddingBits) - 1;
    if ((lastByte & mask) !== 0)
      return { ranges: [], error: SszError.BitlistPadding, msg: 'Bitlist padding non-zero' };
    return { ranges: [{ start: 0, end: bytes.length }], error: SszError.None, msg: '' };
  }
  ...
  if (td.kind === TypeKind.List || td.kind === TypeKind.Vector) {
    if (!td.elementType)
      return {
        ranges: [],
        error: SszError.UnsupportedType,
  ...
  if (td.kind === TypeKind.Container) {
    if (!td.fieldTypes || td.fieldTypes.length === 0)
      return { ranges: [{ start: 0, end: bytes.length }], error: SszError.None, msg: '' };
    const fixedFields: boolean[] = td.fieldTypes.map(
      (ft) => ft.fixedSize !== undefined && ft.fixedSize > 0
    );
    ...
    if (varCount === 0) {
      ...
      if (bytes.length !== totalFixedSize)
        return { ranges: [], error: SszError.MalformedHeader, msg: 'Container length mismatch' };
      ...
      if (offset !== bytes.length)
        return { ranges: [], error: SszError.NonCanonical, msg: 'Container trailing bytes' };
      return { ranges, error: SszError.None, msg: '' };
    } else {
...
function parseVariableList(bytes: Uint8Array): { ranges: Range[]; error: SszError; msg: string } {
  if (bytes.length < 4)
    return {
      ranges: [],
      error: SszError.MalformedHeader,
  ...
  if (offsets.length === 0)
    return { ranges: [], error: SszError.MalformedHeader, msg: 'No offsets found' };
  const headerEnd = offsets[0];
  if (headerEnd !== offsets.length * 4)
    return { ranges: [], error: SszError.BadOffset, msg: 'Offset table misalignment' };
  for (let j = 1; j < offsets.length; j++) {
    if (offsets[j] <= offsets[j - 1])
      return { ranges: [], error: SszError.BadOffset, msg: 'Offsets not strictly increasing' };
  }
  if (offsets[offsets.length - 1] > bytes.length)
    return { ranges: [], error: SszError.LengthOverflow, msg: 'Offset beyond buffer' };
  if (offsets[offsets.length - 1] !== bytes.length)
    return { ranges: [], error: SszError.NonCanonical, msg: 'Trailing bytes in list' };
  const ranges: Range[] = [];
...
function parseVariableContainer(
  td: TypeDesc,
  bytes: Uint8Array,
  fixedFields: boolean[]
): { ranges: Range[]; error: SszError; msg: string } {
  ...
  if (bytes.length < fixedSize)
    return { ranges: [], error: SszError.MalformedHeader, msg: 'Container too short' };
  const offsets: number[] = [];
  let byteOffset = 0;
  for (let i = 0; i < fieldCount; i++) {
    if (fixedFields[i]) {
  ...
  for (const off of offsets) {
    if (off < headerEnd)
      return { ranges: [], error: SszError.BadOffset, msg: 'Offset points into header' };
    if (off > bytes.length)
      return { ranges: [], error: SszError.LengthOverflow, msg: 'Offset beyond buffer' };
  }
