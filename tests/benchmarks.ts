...
/* Performance benchmarks for SSZ operations */
...
// Benchmark: uint64
{
  ...
  const result = benchmark('uint64 root computation', 10000, () => {
    sszStreamRootFromSlice(uint64Type, data);
  });
  printResult(result);
}
...
// Benchmark: bytes32
{
  ...
  const result = benchmark('bytes32 root computation', 10000, () => {
    sszStreamRootFromSlice(bytes32Type, data);
  });
  printResult(result);
}
...
// Benchmark: Small list (10 elements)
{
  ...
  const result = benchmark('list<uint64>[10] root computation', 5000, () => {
    sszStreamRootFromSlice(listUint64, data);
  });
  printResult(result);
}
...
// Benchmark: Small bitlist
{
  ...
  const result = benchmark('bitlist[8] root computation', 5000, () => {
    sszStreamRootFromSlice(bitlistType, data);
  });
  printResult(result);
}
