import { sszStreamRootFromSlice, TypeDesc, TypeKind, SszError } from '../src/index.js';
...
const uint16Type: TypeDesc = { kind: TypeKind.Basic, fixedSize: 2 };
const uint32Type: TypeDesc = { kind: TypeKind.Basic, fixedSize: 4 };
...
// Test: All basic type sizes
{
  ...
  data1[0] = 0xff;
  const res1 = sszStreamRootFromSlice(uint8Type, data1);
...

{
  ...
  data.fill(0xff);
  const res = sszStreamRootFromSlice(uint32Type, data);
...

{
  ...
  data.fill(0xaa);
  const res = sszStreamRootFromSlice(uint256Type, data);
...
const listUint32: TypeDesc = { kind: TypeKind.List, elementType: uint32Type };

{
  // Empty list
  const data = new Uint8Array(0);
  const res = sszStreamRootFromSlice(listUint8, data);
...

{
  ...
  for (let i = 0; i < 33; i++) data[i] = i & 0xff;
  const res = sszStreamRootFromSlice(listUint8, data);
...

{
  ...
  for (let i = 0; i < 1023; i++) data[i] = i & 0xff;
  const res = sszStreamRootFromSlice(listUint8, data);
...
const vectorUint64: TypeDesc = { kind: TypeKind.Vector, elementType: uint64Type };

{
  // Vector with 1 element
  const data = new Uint8Array(8);
  data[0] = 0x42;
  const res = sszStreamRootFromSlice(vectorUint64, data);
...
// Test: Wrong sizes
{
  ...
  const res = sszStreamRootFromSlice(uint64Type, data);
  assert('error' in res && res.error === SszError.NonCanonical, 'uint64 wrong size should fail');
}

{
  const data = new Uint8Array(9); // should be 8
  const res = sszStreamRootFromSlice(uint64Type, data);
  assert('error' in res && res.error === SszError.NonCanonical, 'uint64 extra byte should fail');
}

{
  const data = new Uint8Array(31); // should be 32
  const res = sszStreamRootFromSlice(bytes32Type, data);
  assert('error' in res && res.error === SszError.NonCanonical, 'bytes32 wrong size should fail');
}
...
// Test: List alignment issues
{
  ...
  const res = sszStreamRootFromSlice({ kind: TypeKind.List, elementType: uint64Type }, data);
  assert('error' in res && res.error === SszError.NonCanonical, 'misaligned list should fail');
}
...

{
  ...
  const res = sszStreamRootFromSlice(containerType, data);
  assert('error' in res && res.error === SszError.BadOffset, 'offset into header should fail');
}
